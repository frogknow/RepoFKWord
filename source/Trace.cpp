...
const TraceMap& Trace::GetTraceMap()
{
...
sint Trace::GetSysLogLvl(uint lvl)
{
...
void Trace::ListAll(ostream& os)
{
	...
	for(const auto& it : trcMap){
		if(it.second != nullptr){
			os << std::setw(15) << it->first << "(" << GetTrcLvlName(it->second->GetLevel());
		}
...
void Trace::DoSetLevel(const char* trcName, const char* strLvl, ostream& os)
{
	Trace* pTrace = FindTrace(trcName);
	if(pTrace == nullptr){
		os << "Error: trace " << trcName << "is invalid, use trc list to check " << end;
		return;
	}
